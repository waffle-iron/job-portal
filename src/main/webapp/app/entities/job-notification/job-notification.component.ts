...
export class JobNotificationComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.jobNotificationService.search({
    ...
    trackId(index: number, item: JobNotification) {
        return item.id;
    }
    ...
    registerChangeInJobNotifications() {
        this.eventSubscriber = this.eventManager.subscribe('jobNotificationListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.jobNotifications.push(data[i]);
        }
    }
